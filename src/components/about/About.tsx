...
const Timeline = memo(() => {
  ...
  return (
    <div className="mt-8 space-y-6">
      <Reveal>
        <h3 className="text-lg font-medium text-green-600 dark:text-green-500 mb-4">Професионален път</h3>
      </Reveal>
      ...
      <div className="relative">
        ...
        {timelineItems.map((item, index) => (
          <Reveal key={`timeline-${index}`}>
            ...
            >
              {/* Timeline dot */}
              <div className="absolute left-0 top-1 w-12 h-12 rounded-full bg-zinc-100 dark:bg-zinc-800 border border-green-500/30 flex items-center justify-center z-10">
                {item.icon}
              </div>
              
              <div className="bg-zinc-50/60 dark:bg-zinc-800/30 rounded-lg p-4 border border-zinc-200/60 dark:border-zinc-700/20">
                <span className="text-xs font-semibold inline-block py-1 px-2 uppercase rounded-full bg-green-100 dark:bg-green-500/10 text-green-600 dark:text-green-500 mb-1">
                  {item.year}
                </span>
                <h4 className="text-zinc-800 dark:text-zinc-200 font-medium">{item.title}</h4>
...
const ResumeDownload = memo(() => {
  return (
    <Reveal>
      <div className="mt-6 p-4 bg-zinc-50/60 dark:bg-zinc-800/30 rounded-lg border border-zinc-200/60 dark:border-zinc-700/30">
        <h4 className="font-medium text-green-600 dark:text-green-500 mb-3">Изтеглете CV</h4>
        <a 
          href="/resume.pdf" 
          target="_blank"
          rel="noopener noreferrer"
          className="flex items-center justify-center gap-2 bg-zinc-100 hover:bg-zinc-200 dark:bg-zinc-800 dark:hover:bg-zinc-700 text-zinc-800 dark:text-zinc-200 py-2 px-4 rounded-md transition-colors w-full"
        >
          <MdLibraryBooks />
...
const SocialLinksSection = memo(() => {
  return (
    <Reveal>
      <div className="flex items-center gap-6 mt-6 p-3 border border-zinc-200/50 dark:border-zinc-800/50 rounded-lg bg-zinc-50/50 dark:bg-zinc-900/30 backdrop-blur-sm">
        ...
        <div className="space-y-4">
          <Reveal>
            <div className="relative overflow-hidden rounded-lg mb-6">
              ...
              <p className="leading-relaxed text-zinc-700 dark:text-zinc-300 relative z-10 p-4">
